...
Optional — override up to 10 brand colors with a JSON env var. Unset keys keep their defaults. **Wrap the value in single quotes** — every hex starts with `#`, which dotenv otherwise treats as a comment and silently truncates.
